...
public class Hospital extends Model {
    ...
    private Queue<Patient> queuePatient;            // Fila de pacientes à espera
     ...
     */
    public void attendPacientReceptionist(Patient patient, Receptionist receptionist) {
        // Verifica se o recepcionista está disponível
        if (receptionist.getIsAvailable()) {
            ...
            event.schedule(new TimeSpan(10));
            ...
            // Se a recepcionista não estiver disponível, o paciente entra na fila da recepcionista
            if (receptionist.getWaitingQueue().length() <= maxQueueSize) {
                receptionist.getWaitingQueue().insert(patient);
                sendTraceNote("Paciente adicionado à fila da recepcionista. Fila da Recepção: " + receptionist.getWaitingQueue().size());
            } else {
    ...
    // public void freePacientReceptionist(Patient patient, Receptionist receptionist){
    //     if(receptionist == null) throw new IllegalStateException("A recepcionista nao pode ser nula.");
    //     ReceptionEndEvent event = new ReceptionEndEvent(getModel(), uuid, bShowInTrace);
    //     event.schedule(patient, receptionist, new TimeSpan(0));
    //     sendTraceNote("Fila da Recepçao: " + queuePatient.size());
    // }
